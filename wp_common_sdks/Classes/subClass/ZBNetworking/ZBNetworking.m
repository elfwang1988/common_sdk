...
#define TIMEOUT 10.f //请求超时时间
#define baseUrl @"http://192.168.1.4:8080" //请求baseUrl
...
- (ZBURLSessionTask *)getWithUrl:(NSString *)url cache:(BOOL)cache params:(NSDictionary *)params progressBlock:(ZBGetProgress)progressBlock successBlock:(ZBResponseSuccessBlock)successBlock failBlock:(ZBResponseFailBlock)failBlock {
    ...
    session = [manager GET:url parameters:params progress:^(NSProgress * _Nonnull downloadProgress) {
        ...
        NSNumber * num =  [responseDic objectForKey:@"success"];
        NSString * message =  [responseDic objectForKey:@"message"];
        if ([num isEqualToNumber:@0]) {
            [JL_AlertTool showAlertTips:message onVC:nil];
        ...
        [JL_AlertTool showAlertTips:errorStr onVC:nil];
